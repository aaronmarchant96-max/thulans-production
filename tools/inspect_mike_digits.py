...
for prefix in ["Index", "Ring", "Pinky", "Thumb"]:
    for seg in ["1.R", "2.R"]:
        vg_name = f"{prefix}{seg}"
        ...
        if vg:
            ...
            if v_indices:
                ...
                print(f"{vg_name:10}: {len(v_indices)} verts, bounds min={tuple(round(x,3) for x in mn)} max={tuple(round(x,3) for x in mx)} size={tuple(round(x,3) for x in (mx-mn))}")
