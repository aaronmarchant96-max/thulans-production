#!/usr/bin/env python3
"""Independent, read-only validation for a frozen Varek Chassis V2A.1 blend."""
...
PLAN_PATH = ROOT / "docs" / "VAREK_CHASSIS_V2A1_REVISION_PLAN.md"
BUILDER_PATH = ROOT / "tools" / "build_varek_chassis_v2a1.py"
V2_MEASUREMENTS = ROOT / "evidence" / "varek-chassis-v2a" / "measurements.json"
DEFAULT_MEASUREMENTS = ROOT / "evidence" / "varek-chassis-v2a1" / "measurements.json"
DEFAULT_OUTPUT = ROOT / "evidence" / "varek-chassis-v2a1" / "validation.json"
...
def main():
    ...
    if not bpy.data.filepath:
        raise RuntimeError("validator requires an opened V2A.1 candidate")
    blend = Path(bpy.data.filepath).resolve()
    contract = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
    measurements = json.loads(args.measurements.read_text(encoding="utf-8"))
    v2_measurements = json.loads(V2_MEASUREMENTS.read_text(encoding="utf-8"))
    ...
    checks.append(item("negative_fixture.verdict_sha256", digest(ROOT / contract["negative_fixture"]["visual_verdict_path"]) == contract["negative_fixture"]["visual_verdict_sha256"], digest(ROOT / contract["negative_fixture"]["visual_verdict_path"]), contract["negative_fixture"]["visual_verdict_sha256"]))
    checks.append(item("procedural_helper.sha256", digest(ROOT / contract["procedural_helper"]["path"]) == contract["procedural_helper"]["sha256"], digest(ROOT / contract["procedural_helper"]["path"]), contract["procedural_helper"]["sha256"]))
    ...
    importance_counts = {key: sum(1 for obj in production if obj.get("form_importance") == key) for key in ("PRIMARY", "SECONDARY", "TERTIARY")}
    v2_tertiary = v2_measurements["gates"]["tertiary_ceiling"]["count"]
    tertiary = importance_counts["TERTIARY"]
    checks.append(item("forms.tertiary_not_increased", tertiary <= v2_tertiary and tertiary <= contract["tertiary_object_max"], tertiary, {"v2_count_max": v2_tertiary, "contract_max": contract["tertiary_object_max"]}))
    ...
    for first_name, second_name in contract["structural_junctions"]:
        ...
        junction_readback.append({"objects": [first_name, second_name], "aabb_gap_m": gap})
    failing_junctions = [entry for entry in junction_readback if entry["aabb_gap_m"] > contract["structural_junction_max_gap_m"]]
    checks.append(item("load_path.junction_proximity", not failing_junctions, junction_readback, {"maximum_gap_m": contract["structural_junction_max_gap_m"]}))

    provisional = measurements.get("provisional_proportion_readback", {})
    head_method = provisional.get("methods", {}).get("head_clearance", "")
    head_value = provisional.get("head_to_yoke_local_overhead_clearance_m")
    checks.append(item("proportions.head_clearance_method", head_value is not None and "within operator-cell" in head_method, {"value_m": head_value, "method": head_method}, "local overhead sample within operator-cell X band"))
    checks.append(item("proportions.measurement_class", provisional.get("measurement_class") == contract["head_clearance"]["measurement_class"], provisional.get("measurement_class"), contract["head_clearance"]["measurement_class"]))
    ...
    output = {
        ...
        "gate": "VAREK_CHASSIS_V2A1_INDEPENDENT_VALIDATION",
        ...
        "readback": {
            ...
            "form_object_counts": importance_counts,
            "external_asset_count": len(external),
            "structural_junctions": junction_readback,
            "proportion_measurement_class": provisional.get("measurement_class"),
    ...
    if failures:
        print("VAREK_CHASSIS_V2A1_VALIDATION_FAIL=" + ",".join(failures), file=sys.stderr)
        raise SystemExit(1)
    print(f"VAREK_CHASSIS_V2A1_VALIDATION_PASS={before_sha}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"VAREK_CHASSIS_V2A1_VALIDATION_ERROR={exc}", file=sys.stderr)
        raise SystemExit(1)
